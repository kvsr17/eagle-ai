...
export function Navbar() {
  ...
  return (
    <nav className="bg-background text-foreground shadow-md no-print border-b border-border">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between h-16"> {/* Increased height to h-16 */}
        <Link href="/" className="flex items-center gap-3 text-xl font-semibold text-primary hover:opacity-90 transition-opacity">
          <Scale size={28} /> {/* Icon size increased */}
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {loading ? (
            ...
            <>
              {/* Login and Signup buttons removed for unauthenticated users */}
            </>
          )}
        </div>
      </div>
